...
func miniMaxMusicModels() []systemModelTemplate {
	return []systemModelTemplate{
		...
		miniMaxMusicGenerationTemplate(systemModelIdentity{upstreamID: "music-3.0-free", displayName: "MiniMax Music 3.0 Free"}),
...
func miniMaxMusicCoverTemplate() systemModelTemplate {
	...
	evidence := miniMaxMusicEvidence()
	return systemModelTemplate{
		upstreamID: "music-cover", displayName: "MiniMax Music Cover", inputModalities: []string{"text"}, reasoning: catalog.CapabilityUnsupported, toolCalling: catalog.CapabilityUnsupported, parallelTools: catalog.CapabilityUnsupported, streamingTools: catalog.CapabilityUnsupported, strictSchema: catalog.CapabilityUnsupported, entitlementMode: catalog.EntitlementAllBoundCredentials,
		operation: vcp.OperationMusicCover, actionBindingID: providerminimax.MusicCoverActionBindingID,
		mediaOutputs: []catalog.MediaOutputCapability{miniMaxMusicOutputCapability(evidence)},
		parameters: []catalog.ParameterDescriptor{
			{ID: "preparation_id", Kind: catalog.ParameterString, Required: true, StringRange: &catalog.StringRange{}},
			...
			{ID: "lyrics", Kind: catalog.ParameterString, Required: true, StringRange: &catalog.StringRange{MinimumLength: &minimumLyrics, MaximumLength: &maximumLyrics}},
			{ID: "output_format", Kind: catalog.ParameterFormat, AllowedValues: []string{"mp3", "wav", "pcm"}},
		},
		usageMetrics: []catalog.UsageMetricCapability{{Unit: catalog.UsageUnitAudioMilliseconds, Accuracy: catalog.UsageExact}},
	}
