...
func newServer(catalog ProviderCatalog, control *ControlPlane) (*Server, error) {
	...
	if control != nil {
		...
		mux.Handle("DELETE /vulcan/manage/provider-definitions/{provider_definition_id}", server.requireManagement(http.HandlerFunc(server.handleDeleteCustomDefinition)))
		...
		mux.Handle("POST /vulcan/manage/provider-configurations", server.requireManagement(http.HandlerFunc(server.handleConfigureProvider)))
		mux.Handle("DELETE /vulcan/manage/provider-instances/{provider_instance_id}", server.requireManagement(http.HandlerFunc(server.handleDeleteProviderConfiguration)))
		...
		mux.Handle("POST /vulcan/manage/provider-instances/{provider_instance_id}/credentials/attach", server.requireManagement(http.HandlerFunc(server.handleAttachCredential)))
