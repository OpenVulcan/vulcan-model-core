...
func TestQwenImageGenerationUsesSynchronousMultimodalContract(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if upstream.Model != "qwen-image-2.0-pro" || len(upstream.Input.Messages) != 1 || len(upstream.Input.Messages[0].Content) != 1 || upstream.Input.Messages[0].Content[0].Text != "A blue square" || upstream.Parameters.Size != "1536*1536" || upstream.Parameters.Count != 2 || upstream.Parameters.NegativePrompt != "blur" {
	...
	execution.Execution.Payload.ImageGenerate = &vcp.ImageGenerateOperation{Prompt: "A blue square", NegativePrompt: "blur", Count: 2, Width: 1536, Height: 1536, OutputFormat: "png"}
...
func TestQwenImageEditPreservesOrderedInlineAndRemoteSources(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if len(content) != 3 || content[0].Image != "data:image/png;base64,c291cmNl" || content[1].Image != "https://inputs.example/reference.webp" || content[2].Text != "Combine the subjects" || upstream.Parameters.Count != 1 {
	...
	execution.Execution.Payload.ImageEdit = &vcp.ImageEditOperation{Instruction: "Combine the subjects", Count: 1, OutputFormat: "png", Sources: []vcp.MediaInput{
